...
import phase3.project.model.Product;
import phase3.project.model.Purchase;
...
public class DAO {
	...
	public  String authenticate(String user, String password) {
	 MapSqlParameterSource namedParameters =new MapSqlParameterSource().addValue("user",user);
	return password;
	}
	...
	public List<Purchase> searchPurchasesByDate(String date){
		return jdbcTemplate.query("select u.name, p.name, p.category,"
				+ " pu.ts from purchase pu left join (user u, product p) "
				+ "on (pu.name_id = u.id and pu.product_id = p.id) where pu.ts=?" ,
	...
	public List<Purchase> searchPurchasesByCategory(String category){
		return jdbcTemplate.query("select u.name, p.name, p.category, "
				+ "pu.ts from purchase pu left join (user u, product p) "
				+ "on (pu.name_id = u.id and pu.product_id = p.id) where p.category=?" ,
				new PreparedStatementSetter() {
